...
class MYSQL_DUMP
{
    ...
    public function dumpDatabases()
    {
        ...
        $this->backupFormat = date('Y-m-d-H-i-s');
        ...
        while ($row = $q->fetch_array(MYSQLI_NUM)) {
            if (preg_match('/^'.$this->includeDatabases.'$/', $row[0])) {
                if (preg_match('/^'.$this->ignoreDatabases.'$/', $row[0])) {
                    ...
                    $this->debug('- Database: '.json_encode($row));
        ...
        exec('cd '.$this->backupDir.' ; '.$this->tar_binary.' -zvcf '.$this->backupDir.'/'.$this->backupFormat.'.tar.gz '.$this->backupFormat.' > /dev/null');
        // echo 'cd ' . $this->backupDir . ' ; ' . $this->tar_binary . ' -zcf ' . $this->backupDir . '/' . $this->backupFormat . '.tar.zip ' . $this->backupFormat . ' > /dev/null';
        chmod($this->backupDir.'/'.$this->backupFormat.'.tar.gz', $this->savePermissions);
